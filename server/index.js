...
const server=http.createServer(async(req,res)=>{
  try {
    ...
    if(req.method==='GET'&&pathname==='/api/qr'){const base=`${url.origin}/surprise`;const png=await QRCode.toBuffer(base,{type:'png',width:512,margin:2,errorCorrectionLevel:'H'});return send(res,200,png,'image/png',{'Content-Length':png.length});}
