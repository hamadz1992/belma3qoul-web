...
function Admin(){
 ...
 const [form,setForm]=useState({name:'',type:'voucher',value:'',quantity:1,weight:10,link:'',enabled:true});
 ...
 const campaignForm=<>
   ...
   <form className="adminCard formCard" onSubmit={addPrize}><h2>🎁 الجوائز في نفس المفاجأة</h2><div className="two"><label>اسم الجائزة<input required value={form.name} onChange={e=>setForm({...form,name:e.target.value})}/></label><label>النوع<select value={form.type} onChange={e=>setForm({...form,type:e.target.value})}><option value="voucher">قسيمة شراء</option><option value="link">رابط</option><option value="product">جائزة عينية</option><option value="message">رسالة</option></select></label></div><div className="two"><label>القيمة/الوصف<input value={form.value} onChange={e=>setForm({...form,value:e.target.value})}/></label><label>عدد الفائزين<input type="number" min="0" required value={form.quantity} onChange={e=>setForm({...form,quantity:e.target.value})}/></label></div><div className="two"><label>وزن الجائزة في السحب<input type="number" min="0" value={form.weight} onChange={e=>setForm({...form,weight:e.target.value})}/></label><label>رابط الجائزة (اختياري)<input value={form.link} onChange={e=>setForm({...form,link:e.target.value})}/></label></div><button className="primary" type="submit">+ إضافة الجائزة</button></form>
   <div className="adminCard"><h2>📦 الجوائز الحالية</h2><div className="prizeList">{data.prizes.length?data.prizes.map(p=><div className="prizeRow" key={p.id}><span className="prizeIcon">🎁</span><div><b>{p.name}</b><small>{TYPES[p.type]||p.type} • الوزن {p.weight}</small></div><strong>{p.remaining}/{p.quantity}</strong><button onClick={()=>editPrize(p)}>تعديل العدد</button><button onClick={()=>deletePrize(p)}>حذف</button></div>):<p>لا توجد جوائز مضافة بعد.</p>}</div></div>
   <div className="adminCard formCard saveSettingsCard"><h2>💾 حفظ الإعدادات</h2><p>احفظ جميع إعدادات المفاجأة دفعة واحدة بعد الانتهاء من التعديلات.</p><button className="primary full" onClick={saveAll}>{saveState==='saving'?'جارٍ الحفظ…':saveState==='saved'?'✓ تم حفظ جميع الإعدادات':saveState==='error'?'إعادة المحاولة':'حفظ جميع الإعدادات'}</button></div>
 </>;
 return <div className="admin"><aside><a href="/admin" className="sideLogo"><Logo/></a><button className={tab==='overview'?'active':''} onClick={()=>setTab('overview')}>📊 نظرة عامة</button><button className={tab==='campaign'?'active':''} onClick={()=>setTab('campaign')}>🎯 الحملة والمفاجأة</button><button className={tab==='plays'?'active':''} onClick={()=>setTab('plays')}>🏆 الفائزون والمشاركات</button><a href="/">🌐 عرض الموقع</a><button className="logout" onClick={logout}>↪ تسجيل الخروج</button></aside><main className="adminMain"><header className="adminHeader"><div><span className="eyebrow">BELMA3QOUL ADMIN</span><h1>{tab==='overview'?'نظرة عامة':tab==='campaign'?'الحملة والمفاجأة':'المشاركات والفائزون'}</h1></div><span className="online">● النظام متصل</span></header>
